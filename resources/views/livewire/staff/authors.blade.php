...
new #[Layout('layouts.dashboard')] class extends Component {
    ...
    {
        $data = $this->validate(['nama' => ['required', 'string', 'max:255']]);
        Author::updateOrCreate(['id' => $this->editingId], $data);
        $this->showForm = false;
        $this->dispatch('toast', type: 'success', message: 'Penulis disimpan.');
    }

    public function delete(int $id): void
    {
        $a = Author::withCount('books')->findOrFail($id);
        if ($a->books_count > 0) {
            $this->dispatch('toast', type: 'error', message: 'Tidak bisa dihapus, masih dipakai buku.');
            return;
        }
        $a->delete();
        $this->dispatch('toast', type: 'success', message: 'Penulis dihapus.');
    }

    public function with(): array
    {
        return [
            'items' => Author::withCount('books')
...
<div>
    <div class="mb-4 flex items-center justify-between gap-3">
        <div class="relative max-w-sm flex-1">
            ...
            <input wire:model.live.debounce.400ms="search" type="text" placeholder="Cari penulis…"
    ...
    <div class="overflow-hidden rounded-xl border bg-white shadow-sm">
        <table class="min-w-full divide-y text-sm">
            <thead class="bg-gray-50 text-left text-xs uppercase tracking-wider text-gray-500">
                <tr><th class="px-4 py-3">Nama</th><th class="px-4 py-3">Buku</th><th class="px-4 py-3 text-right">Aksi</th></tr>
            </thead>
            <tbody class="divide-y">
                @forelse ($items as $a)
                    <tr class="hover:bg-gray-50">
                        <td class="px-4 py-3 font-medium text-gray-800">{{ $a->nama }}</td>
                        <td class="px-4 py-3">{{ $a->books_count }}</td>
                        <td class="px-4 py-3">
                            <div class="flex justify-end gap-2">
                                <button wire:click="edit({{ $a->id }})" class="rounded-md border px-2.5 py-1 text-xs hover:bg-gray-50">Edit</button>
                                <button wire:click="delete({{ $a->id }})" wire:confirm="Hapus penulis ini?" class="rounded-md border border-rose-200 px-2.5 py-1 text-xs text-rose-600 hover:bg-rose-50">Hapus</button>
                            </div>
                        </td>
                    </tr>
                @empty
                    <tr><td colspan="3" class="px-4 py-10 text-center text-gray-400">Tidak ada data.</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
    ...
    @if ($showForm)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
                <h3 class="mb-4 text-lg font-semibold text-gray-800">{{ $editingId ? 'Edit' : 'Tambah' }} Penulis</h3>
