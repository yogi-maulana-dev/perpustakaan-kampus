...
new #[Layout('layouts.dashboard')] class extends Component {
    ...
    public function save(): void
    {
        ...
        Publisher::updateOrCreate(['id' => $this->editingId], $data);
        $this->showForm = false;
        $this->dispatch('toast', type: 'success', message: 'Penerbit disimpan.');
    }

    public function delete(int $id): void
    {
        $p = Publisher::withCount('books')->findOrFail($id);
        if ($p->books_count > 0) {
            $this->dispatch('toast', type: 'error', message: 'Tidak bisa dihapus, masih dipakai buku.');
            return;
        }
        $p->delete();
        $this->dispatch('toast', type: 'success', message: 'Penerbit dihapus.');
    }
...
<div>
    <div class="mb-4 flex items-center justify-between gap-3">
        <div class="relative max-w-sm flex-1">
            ...
            <input wire:model.live.debounce.400ms="search" type="text" placeholder="Cari penerbit…"
            ...
            <tbody class="divide-y">
                @forelse ($items as $p)
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-4 py-3">{{ $p->books_count }}</td>
                        <td class="px-4 py-3">
                            <div class="flex justify-end gap-2">
                                <button wire:click="edit({{ $p->id }})" class="rounded-md border px-2.5 py-1 text-xs hover:bg-gray-50">Edit</button>
                                <button wire:click="delete({{ $p->id }})" wire:confirm="Hapus penerbit ini?" class="rounded-md border border-rose-200 px-2.5 py-1 text-xs text-rose-600 hover:bg-rose-50">Hapus</button>
                            </div>
                        </td>
                    </tr>
                @empty
    ...
    @if ($showForm)
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
            <div class="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
                <h3 class="mb-4 text-lg font-semibold text-gray-800">{{ $editingId ? 'Edit' : 'Tambah' }} Penerbit</h3>
